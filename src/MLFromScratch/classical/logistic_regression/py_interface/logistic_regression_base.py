from ...linear_regression.py_interface.linear_regression_base import (
...
class LogisticRegressionBase(LinearRegressionBase):
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray) -> np.ndarray:
        ...
        if self.params["num_classes"] == 1:
            raise ValueError("Only one class found in Y. Please check the target values.")
        ...
        if not np.array_equal(np.unique(Y), np.arange(self.params["num_classes"])):
            raise ValueError("Output classes should be from 0 to num_classes-1.")
